...
public class RdsManager {
    ...
    public interface RdsCallback {
        ...
        /**
         * Default no-op so existing implementers stay valid. Wird gefeuert wenn
         * der FM-Chip auf eine alternative Frequenz (AF) geschaltet hat.
         * rawFreq ist der Rohwert vom Chip (typ. MHz×10).
         */
        default void onAfSwitch(int rawFreq) {}
    }
    ...
    public RdsManager(FmNative fmNative) {
        ...
        com.android.fmradio.FmService.setRdsListener((eventType, p1, p2, p3) -> {
            if (eventType == com.android.fmradio.FmService.RDS_CMD_NOTIFY_AF_SWITCH) {
                final int raw = p1;
                final RdsCallback cb = callback;
                if (cb != null) {
                    handler.post(() -> cb.onAfSwitch(raw));
                }
            }
        });
    }
    ...
    private boolean enableRdsViaRootFallback() {
        if (!SqlFMServiceClient.isRootFallbackAllowed() || !SqlFMServiceClient.isRootAvailable()) {
            return false;
        }
    ...
    private boolean tuneViaRootFallback(float frequency) {
        if (!SqlFMServiceClient.isRootFallbackAllowed() || !SqlFMServiceClient.isRootAvailable()) {
            Log.w(TAG, "Root-Fallback nicht erlaubt oder nicht verfügbar");
            return false;
        }
