...
class RdsLogAdapter : RecyclerView.Adapter<RdsLogAdapter.ViewHolder>() {
    ...
    inner class ViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        fun bind(entry: RdsLogEntry) {
            ...
            // RSSI
            tvRssi.text = ctx.getString(R.string.rssi_format, entry.rssi)

            // PI
            if (entry.pi != 0) {
                tvPi.text = ctx.getString(R.string.pi_format, entry.pi and 0xFFFF)
            ...
            // PTY
            if (entry.pty > 0) {
                val ptyNames = ctx.resources.getStringArray(R.array.rds_pty_names)
                val ptyName = ptyNames.getOrNull(entry.pty) ?: ctx.getString(R.string.unknown)
                tvPty.text = ctx.getString(R.string.pty_format, ptyName)
            ...
            if (entry.eventType == "STATION_CHANGE") {
                tvEvent.text = ctx.getString(R.string.rds_event_tune)
