...
class DabStripAdapter(
    ...
    /**
     * 0 = nicht gesetzt → Resource-Default (R.color.radio_accent) wird benutzt.
     * Sonst wird der hier abgelegte ARGB-Wert für den selected-Card-Hintergrund
     * verwendet. Setter triggert Repaint.
     */
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        val bgColor = if (isSelected) {
            if (accentColorOverride != 0) accentColorOverride
            else ContextCompat.getColor(holder.itemView.context, R.color.radio_accent)
        } else {
