...
class StationCarouselAdapter(
    ...
    /** 0 = no override, sonst ARGB-Tint für die Placeholder-Vektoren
     *  (FM/AM/DAB-Branding-Logos), damit die in der App-Akzentfarbe
     *  rendern. Echte Cover-Bilder bleiben untintet. */
    private var placeholderTint: Int = 0

    fun setAccentColor(color: Int) {
        if (placeholderTint == color) return
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        if (isSelected) {
            ...
            if (!coverSource.isNullOrBlank()) {
                applyTintForRealCover(holder.stationLogo)
                ...
                applyTintForPlaceholder(holder.stationLogo)
            }
        } else {
    ...
    private fun applyTintForPlaceholder(view: ImageView) {
        // Theme-konformer Look: Hintergrund = `radio_background` (hell im
        // Day, dunkel im Night), Logo-Pfade = `radio_text_primary` (Kontrast
        // zum BG, je Theme). Folgt automatisch dem Day/Night-Switch.
        val ctx = view.context
        view.setBackgroundColor(
            ContextCompat.getColor(ctx, R.color.radio_background)
        )
        androidx.core.widget.ImageViewCompat.setImageTintList(
            view,
            android.content.res.ColorStateList.valueOf(
                ContextCompat.getColor(ctx, R.color.radio_text_primary)
            )
        )
    }

    private fun applyTintForRealCover(view: ImageView) {
        // Echte JPG/PNG Cover dürfen nicht tintiert werden — sonst wird das
        // ganze Bild eingefärbt. Tint und farbiger BG löschen.
