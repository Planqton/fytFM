...
class StationChangeOverlayService : Service() {
    ...
    private fun parseStations(json: String) {
        try {
            ...
            android.util.Log.e(TAG, "Error parsing stations: ${e.message}", e)
        }
    }
    ...
    private fun showDabOverlay(serviceId: Int, oldServiceId: Int = 0) {
        handler.post {
            try {
                ...
                android.util.Log.e(TAG, "Error showing DAB overlay: ${e.message}", e)
            }
        }
    }
    ...
    inner class OverlayCarouselAdapter : RecyclerView.Adapter<OverlayCarouselAdapter.ViewHolder>() {
        ...
        override fun onBindViewHolder(holder: ViewHolder, position: Int) {
            ...
            if (station.isDab) {
                holder.frequencyText.text = station.name ?: ctx.getString(R.string.band_dab_plus)
                holder.bandLabel.text = ctx.getString(R.string.band_dab_plus)
                ...
                holder.bandLabel.text = ctx.getString(if (station.isAM) R.string.band_am else R.string.band_fm)
