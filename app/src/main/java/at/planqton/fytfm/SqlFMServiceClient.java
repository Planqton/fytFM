...
public class SqlFMServiceClient {
    ...
    // Master-Switch via Settings (Default: aus). FytFMApplication setzt den
    // Wert beim App-Start aus den SharedPreferences; SettingsViewModel ruft
    // setRootFallbackAllowed() auf, sobald der User den Toggle umlegt.
    private static volatile boolean sRootFallbackAllowed = false;

    public static void setRootFallbackAllowed(boolean allowed) {
        sRootFallbackAllowed = allowed;
    }

    public static boolean isRootFallbackAllowed() {
        return sRootFallbackAllowed;
    }
    ...
    public boolean tune(float freqMHz) {
        // Root-Fallback Modus (UIS7870/DUDU7) — nur wenn vom User in Settings erlaubt
        if (mUseRootFallback && sRootFallbackAllowed) {
            return tuneViaSu(freqMHz);
        }

        if (!isConnected()) {
            // Versuche Root-Fallback wenn nicht verbunden
            if (sRootFallbackAllowed && isRootAvailable()) {
