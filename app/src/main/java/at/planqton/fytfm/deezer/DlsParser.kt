...
object DlsParser {
    ...
    private fun extractArtistTitle(text: String, original: String, stationName: String?): ParseResult {
        ...
        // Special case: "ARTIST, FULL NAME (YEAR) - Title" (classical music)
        val classicalPattern = Regex("""^([^,]+),\s*([^(]+)\s*\((\d{4}(?:-\d{4})?)\)\s*[-–—]\s*(.+)$""")
        val classicalMatch = classicalPattern.find(text)
        if (classicalMatch != null) {
            val lastName = classicalMatch.groupValues[1].trim()
            val firstName = classicalMatch.groupValues[2].trim()
            val title = classicalMatch.groupValues[4].trim()
    ...
    private fun isStationOrSlogan(text: String, stationName: String?): Boolean {
        ...
        // Multiple station/slogan keywords
        var keywordCount = 0
        for (keyword in STATION_KEYWORDS) {
