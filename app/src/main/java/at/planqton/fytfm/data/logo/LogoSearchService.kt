...
class LogoSearchService {
    ...
    private suspend fun tryRadioDnsLookup(station: RadioStation): String? = withContext(Dispatchers.IO) {
        try {
            ...
            Log.e(TAG, "RadioDNS lookup failed: ${e.message}")
            null
        }
    }
    ...
    private suspend fun tryRadioDnsWithGcc(serviceId: Int, ensembleId: Int, gcc: String): String? {
        try {
            // Format: 0.<sId>.<eId>.<gcc>.dab.radiodns.org
            // Service Component ID is typically 0 for primary audio
            val sIdHex = String.format("%04x", serviceId).lowercase()
            val eIdHex = String.format("%04x", ensembleId).lowercase()
            val fqdn = "0.$sIdHex.$eIdHex.$gcc.dab.radiodns.org"
            ...
            val logo = fetchLogoFromSiXml(siUrl, serviceId)

            if (logo != null) return logo

            // Try alternate SI.xml path
            val siUrlAlt = "http://$cname/radiodns/spi/3.1/SI.xml"
            return fetchLogoFromSiXml(siUrlAlt, serviceId)
    ...
    private fun dohCnameLookup(fqdn: String): String? {
        return try {
            ...
            client.newCall(request).execute().use { response ->
                ...
                val answers = obj.getJSONArray("Answer")
                for (i in 0 until answers.length()) {
                    val answer = answers.getJSONObject(i)
                    // Type 5 = CNAME
                    if (answer.optInt("type") == 5) {
                        return answer.optString("data")?.trimEnd('.')
                    }
                }
            ...
            Log.e(TAG, "DoH lookup failed: ${e.message}")
            null
        }
    }
    ...
    private fun fetchLogoFromSiXml(siUrl: String, serviceId: Int): String? {
        return try {
            ...
            Log.e(TAG, "Failed to fetch SI.xml: ${e.message}")
            null
        }
    }
    ...
    private fun parseLogoFromSiXml(xml: String, serviceId: Int): String? {
        try {
            // Simple XML parsing - look for mediaDescription with logo URLs
            // Format: <mediaDescription><multimedia url="..." width="..." height="..."/></mediaDescription>

            val sIdHex = String.format("%04X", serviceId)
            val sIdHexLower = sIdHex.lowercase()

            // Find the service section for this serviceId
            val servicePattern = Regex("""<service[^>]*>.*?</service>""", RegexOption.DOT_MATCHES_ALL)
            val services = servicePattern.findAll(xml)

            for (service in services) {
                val serviceXml = service.value

                // Check if this service matches our serviceId
                if (serviceXml.contains(sIdHex, ignoreCase = true) ||
                    ...
                    // Extract logo URL - prefer larger sizes
                    val logoPattern = Regex("""<multimedia[^>]*url="([^"]+)"[^>]*(?:width="(\d+)")?""")
                    val logos = logoPattern.findAll(serviceXml).toList()

                    if (logos.isNotEmpty()) {
                        // Sort by width (prefer larger) and return the best one
                        val bestLogo = logos.maxByOrNull { match ->
                            match.groupValues.getOrNull(2)?.toIntOrNull() ?: 0
                        }
                        val logoUrl = bestLogo?.groupValues?.get(1)
                        if (!logoUrl.isNullOrBlank()) {
                            Log.d(TAG, "Found logo in SI.xml: $logoUrl")
                            return logoUrl
                        }
                    }
                }
            ...
            // Fallback: just find any logo URL in the XML
            val anyLogoPattern = Regex("""<multimedia[^>]*url="([^"]+)"[^>]*""", RegexOption.IGNORE_CASE)
            val anyLogo = anyLogoPattern.find(xml)
            val fallbackUrl = anyLogo?.groupValues?.get(1)
            if (!fallbackUrl.isNullOrBlank()) {
                Log.d(TAG, "Found fallback logo in SI.xml: $fallbackUrl")
                return fallbackUrl
            }

            Log.d(TAG, "No logo found in SI.xml")
            return null

        } catch (e: Exception) {
            Log.e(TAG, "Failed to parse SI.xml: ${e.message}")
            return null
        }
    }
    ...
    private suspend fun tryRadioBrowserSearch(stationName: String): String? = withContext(Dispatchers.IO) {
        try {
            ...
            if (searchName.length < 2) return@withContext null

            // Try multiple search strategies
            val searchVariants = listOf(
                searchName,
                searchName.split(" ").firstOrNull() ?: searchName,  // First word only
                searchName.replace(" ", "")  // Without spaces
            ).distinct()
            ...
            Log.e(TAG, "radio-browser.info search failed: ${e.message}")
            null
        }
    }
